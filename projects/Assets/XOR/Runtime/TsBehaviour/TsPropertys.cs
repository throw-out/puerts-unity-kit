...
namespace XOR
{
    ...
    public class TsPropertys : MonoBehaviour
    {
        ...
        [System.Serializable]
        internal class Vector2 : Pair<UnityEngine.Vector2> { }
        [System.Serializable]
        internal class Vector3 : Pair<UnityEngine.Vector3> { }
        [System.Serializable]
...
namespace UnityEditor.GUI
{
    ...
    public class JsPropertys_CustomEditor : Editor
    {
        ...
        void OnEnable()
        {
            //Debug.Log("OnEnable");
            _scriptObject = null;
            _instance = target as XOR.TsPropertys;
            if (_instance == null)
                return;

            //script object
            var targetName = $"/{nameof(XOR.TsPropertys)}.cs";
            ...
            display.AddDrawer<StringDrawer>(typeof(XOR.TsPropertys.String));
            display.AddDrawer<NumberDrawer>(typeof(XOR.TsPropertys.Number));
            display.AddDrawer<BooleanDrawer>(typeof(XOR.TsPropertys.Boolean));
            display.AddDrawer<Vector2Drawer>(typeof(XOR.TsPropertys.Vector2));
            display.AddDrawer<Vector3Drawer>(typeof(XOR.TsPropertys.Vector3));
            display.AddDrawer<ObjectDrawer>(typeof(XOR.TsPropertys.Object));
            display.AddDrawer<ObjectArrayDrawer>(typeof(XOR.TsPropertys.ObjectArray));
            display.AddDrawer<GameObjectArrayDrawer>(typeof(XOR.TsPropertys.GameObjectArray));
        ...
        private DrawerInfo CreateDrawerInfo()
        {
            ...
            if (_instance != null)
            {
                ...
                var fields = typeof(XOR.TsPropertys).GetFields(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
                foreach (var field in fields)
                {
                    Type elementType = field.FieldType.IsArray ? field.FieldType.GetElementType() : null;
                    if (elementType != null
                        && typeof(XOR.TsPropertys.IPair).IsAssignableFrom(elementType)
                        && serializedObject.FindProperty(field.Name) != null
                        && DisplayUtility.CheckFields(elementType))
                    {

                        var options = elementType.GetCustomAttribute<XOR.TsPropertys.OptionsAttribute>(false);
        ...
        void DisplayMenu()
        {
            ...
            if (menuFoldout)
            {
                ...
                GUILayout.Space(5f);
                if (GUILayout.Button("declare code [public]"))
                {
                ...
                if (GUILayout.Button("declare code [protected]"))
                {
                    var code = DisplayUtility.GenCode(_instance.GenPairs(), "declare protected", false);
                    var editor = new TextEditor();
                    editor.text = code;
                    editor.OnFocus();
                    editor.Copy();
                ...
                if (GUILayout.Button("declare code [private]"))
                {
                ...
                {
                    XOR.TsPropertys.ResultPair[] pairs = _instance.GenPairs();
                    if (pairs != null)
                    {
    ...
    public class DisplayUtility
    {
        ...
        {
            FieldInfo index, key;
            return typeof(XOR.TsPropertys.IPair).IsAssignableFrom(type)
        ...
        public static string GenCode(XOR.TsPropertys.ResultPair[] pairs, string declareType, bool useFullname)
        {
            var resultCode = "";
    ...
    class Vector2Drawer : Drawer
    {
        protected override void DrawValue(Rect position, Rect valueRect)
        {
            element.valueNode.vector2Value = EditorGUI.Vector2Field(valueRect, string.Empty, element.valueNode.vector2Value);
        }
    }
    class Vector3Drawer : Drawer
    {
        protected override void DrawValue(Rect position, Rect valueRect)
        {
            element.valueNode.vector3Value = EditorGUI.Vector3Field(valueRect, string.Empty, element.valueNode.vector3Value);
        }
    }
